...
const property estadio = bombonera
var entradasVendidas = 0
var jugadorEstrella = [messi,ronaldo,mbappe]
var property precioEntrada = 10000

method jugadorEstrella() = jugadorEstrella

method recaudacion(){
  entradasVendidas = (jugadorEstrella.sum({jugador => jugador.popularidad()}))* estadio.capacidadEstadio()
  return (entradasVendidas * precioEntrada) - (jugadorEstrella.sum({jugador => jugador.viaticos()})) - estadio.alquiler() - estadio.logistica() 
}

method invitarJugador(jugadorNuevo){

  if(jugadorEstrella.contains(jugadorNuevo) == false)
  jugadorEstrella.add(jugadorNuevo)

}

method retirarJugador(jugadorARetirar){
  if(jugadorEstrella.contains(jugadorARetirar) == true)
  jugadorEstrella.remove(jugadorARetirar)
